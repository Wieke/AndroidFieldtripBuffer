...
public class BufferService extends Service {
	...
	private class BufferMonitor extends Thread implements
			FieldtripBufferMonitor {
		...
		public void updateClientError(final int clientID, final int errorType,
			...
			if (errorType == FieldtripBufferMonitor.ERROR_PROTOCOL) {
				...
				Log.i(C.TAG, "Client " + clientID + " has wrong version "
						+ time + " .");
			} else {
		...
		@Override
		public void updateConnectionOpened(final int clientID,
				final String adress) {
			final Intent intent = new Intent(C.FILTER);
